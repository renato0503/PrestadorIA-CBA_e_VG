...
function loadState() {
    ...
    if (savedState) {
        ...
        // Garantir que lead e metrics existam
...
function askQuestion() {
    ...
    if (currentQuestionIndex < questions.length) {
        ...
        setTimeout(computeAndDisplayPrice, 1500); // Simula processamento
    }
}
...
function setupInputForQuestion(questionData) {
    ...
    userInput.type = questionData.type === 'number' ? 'text' : questionData.type; // Usa 'text' para number para permitir placeholder e validação customizada
    ...
    newSendButton.onclick = () => handleUserResponseWrapper(); // Adiciona novo listener
...
function handleUserResponse(response) {
    ...
    if (validationError) {
        ...
        // Se for select/boolean, a resposta já foi tratada. Se for texto/numérico, mostra erro e aguarda nova entrada.
        if (questionData.type !== 'select' && questionData.type !== 'boolean') {
    ...
    if (validationError && questionData.defaultValue !== undefined) {
        finalResponse = questionData.defaultValue; // Usa default se houve erro na entrada manual
        ...
        finalResponse = questionData.defaultValue; // Usa default se vazio
    } else if (questionData.type === 'text' && finalResponse.trim() === '' && questionData.defaultValue) {
         finalResponse = questionData.defaultValue; // Usa default se vazio
    }


    // Se a validação passou ou a entrada é opcional/default, armazena
    if (!validationError || questionData.defaultValue !== undefined) {
...
function computePrice() {
    ...
    switch (serviceKey) {
        ...
        case 'marceneiro':
            ...
            let surface_m2 = (volume_cm3 / 1000000) * 2; // Simples aproximação: 6 faces / 1.000.000 cm²/m²
            ...
            calculatedPrice = surface_m2 * pricePerCm2 * 100; // Ajuste para preço por m² * 100 (simulando custo base)
            ...
            explanation.push(`Base (R$ ${pricePerCm2.toFixed(2)}/cm² * ${volume_cm3}cm³): R$ ${(surface_m2 * pricePerCm2 * 100).toFixed(2)}`);
            break;
        ...
        case 'pintor':
             ...
             if (answers.tipo_pintura?.toLowerCase().includes('textura')) {
...
function displayPriceAndContinue(priceResult) {
    ...
    if (lastBotMessage) {
        lastBotMessage.appendChild(suggestionsDiv);
         ...
         // Caso não haja última mensagem (improvável), adiciona uma nova
         displayBotMessage("O que deseja fazer agora?", options);
    }

    // Habilitar input novamente, mas com um placeholder para as ações finais
     const userInput = document.getElementById('user-input');
     userInput.style.display = 'block'; // Mostra o input, mas ele não será usado para texto
     userInput.value = '';
     userInput.placeholder = "Escolha uma opção acima...";
     userInput.disabled = true; // Desabilita digitação direta

     const oldSendButton = document.getElementById('send-button');
     const newSendButton = oldSendButton.cloneNode(true);
     oldSendButton.parentNode.replaceChild(newSendButton, oldSendButton);
     newSendButton.onclick = () => {}; // Desabilita o botão de enviar
...
function generateImageSimulation() {
    ...
    switch (serviceKey) {
        case 'pintor':
            ...
            // Simula uma imagem genérica de pintura
            imageUrl = "https://via.placeholder.com/600x400.png?text=Visualizacao+Pintura+Simulada";
            break;
        case 'marmoreiro':
            ...
            // Simula uma imagem de bancada
             imageUrl = "https://via.placeholder.com/600x400.png?text=Visualizacao+Marmore+Simulada";
            break;
    ...
    const apiResponse = {
        success: true, // ou false se houver erro
        imageUrl: imageUrl,
    ...
    if (apiResponse.success && apiResponse.imageUrl) {
        ...
        displayBotMessage("Não foi possível gerar a visualização neste momento. Verifique a qualidade das informações ou tente mais tarde.");
        incrementMetric('errorsEncountered');
    }
}
...
function displayImageSimulation(imageUrl, answers) {
    ...
    // Lógica de sobreposição simples (exemplo para pintor)
    if (conversationState.currentService === 'pintor' && answers.cor_desejada) {
         content += `
            <div style="position: relative; width: 400px; height: 300px; border: 1px solid #ccc; margin: 10px auto; background-color: ${answers.cor_desejada}; color: ${getContrastColor(answers.cor_desejada)}; display: flex; align-items: center; justify-content: center; border-radius: 8px;">
                <p style="background-color: rgba(0,0,0,0.5); padding: 10px; border-radius: 5px;">
                    Ambiente simulado com a cor ${answers.cor_desejada}.<br>
                    ${answers.tipo_pintura ? `Tipo: ${answers.tipo_pintura}` : ''}
                </p>
            </div>
         `;
    } else {
        // Exibe a imagem genérica se não houver lógica específica
        content += `<img src="${imageUrl}" alt="Visualização Simulada" style="max-width: 100%; height: auto; border-radius: 8px; margin-top: 10px;">`;
        if (answers.cor_desejada) {
             content += `<p><small>Cor/Material escolhido: ${answers.cor_desejada}</small></p>`;
        }
    }
...
// Função auxiliar para cor de texto contrastante
...
// Inicializa o chatbot ao carregar a página
window.onload = () => {
    loadState(); // Carrega estado anterior se houver
    loadConfig(); // Carrega configurações e desenha botões de serviço
    updateMetricsDashboard(); // Tenta carregar dashboard de métricas
};
...
Usuário: Clica em [Residencial (Interno)]
...
Bot: (Após cálculo)
     ...
     <strong>Detalhes:</strong> Base (Residencial (Interno)): R$ 2.500,00 | Demãos extras (1): R$ 250,00 | Textura (R$ 15,00/m²): R$ 1.500,00  <-- ERRO NO CÁLCULO SIMULADO - PRECISA CORRIGIR A LÓGICA DE TEXTURA
     <br><br>
     <p class="text-warning">Lembre-se: Este é um orçamento simulado...</p>
     [Salvar Orçamento (Lead)] [Nova Simulação]

Usuário: Clica em [Salvar Orçamento (Lead)]
Bot: (Salva o lead no localStorage)
     (Exibe mensagem de sucesso e o painel de leads)
     Lead salvo com sucesso! Aqui estão seus leads:
     [Painel de Leads...]
     [Nova Simulação]

Usuário: Clica em [Nova Simulação]
Bot: (Reseta o chat)
...
function packageAndSaveLead() {
    ...
    // Cálculo final refeito para inclusão no lead
    let finalPrice = 0;
    let priceExplanation = '';
    let priceRange = { min: 0, max: 0 };

    // Reutiliza a lógica de cálculo para garantir consistência
     try {
         const priceResult = computePriceForLead(); // Função auxiliar para obter o preço sem exibir na tela
         if (priceResult) {
    ...
    const lead = {
        ...
        details: { ...conversationState.answers },
        estimatedPrice: finalPrice,
        priceRange: priceRange,
        priceExplanation: priceExplanation,
        // Adicionar campos para foto e visualização simulada (a serem implementados)
        photoUrl: null, // Placeholder
        visualizationUrl: null // Placeholder
    };

    // Adiciona o novo lead ao array de leads no estado
    conversationState.leads.push(lead);
    saveState(); // Salva o estado com o novo lead
    ...
    // Limpa o estado da conversa atual para iniciar nova simulação
    resetChat();
...
// Função auxiliar para re-calcular preço sem interação
function computePriceForLead() {
     ...
     if (!config || !rules) return null;
      ...
      // Reutiliza a lógica de cálculo interna
     switch (serviceKey) {
         case 'gesseiro':
             let area = parseFloat(answers.area_m2) || 0;
             let pricePerM2 = rules.base_price_per_m2;
         ...
         case 'marceneiro':
             ...
             calculatedPrice = surface_m2 * pricePerCm2 * 100;
             if (answers.cor_acabamento) {
                 calculatedPrice += rules.finishing_fee;
             ...
             explanation.push(`Base (${pricePerCm2.toFixed(2)}/cm² * ${volume_cm3}cm³): R$ ${(surface_m2 * pricePerCm2 * 100).toFixed(2)}`);
             break;
...
function exportLeadAsPdf() {
     ...
     // Placeholder para a implementação futura
      // const leads = conversationState.leads;
...
function displayLeadDashboard() {
    ...
    if (conversationState.leads && conversationState.leads.length > 0) {
        conversationState.leads.forEach(lead => {
            dashboardContent += `
                <div class="lead-item">
                    <h5>Lead #${lead.id} (${lead.service})</h5>
                    ...
                    <p>Detalhes: ${JSON.stringify(lead.details)}</p>
                    <div class="lead-actions">
                        <button class="btn btn-info btn-sm" onclick="viewLeadDetails('${lead.id}')">Ver Detalhes</button>
...
function viewLeadDetails(leadId) {
    ...
    if (lead) {
        // Poderia abrir um modal ou expandir a seção
        alert(`Detalhes do Lead ${leadId}:\nServiço: ${lead.service}\nOrçamento: R$ ${lead.estimatedPrice}\nExplicação: ${lead.priceExplanation}\nRespostas: ${JSON.stringify(lead.details, null, 2)}`);
    }
}
...
function deleteLead(leadId) {
    if (confirm("Tem certeza que deseja excluir este lead?")) {
        ...
        saveState();
        displayLeadDashboard(); // Atualiza a exibição
    }
}
...
function updateMetricsDashboard() {
    ...
    if (!metricsDiv) {
        ...
        // Adiciona o dashboard de métricas antes do dashboard de leads, se ambos existirem
        const leadsDashboard = document.getElementById('lead-dashboard');
        if (leadsDashboard) {
             mainContainer.insertBefore(metricsDiv, leadsDashboard);
        } else {
             mainContainer.appendChild(metricsDiv);
        }
    }

    let metricsContent = `<h2>Métricas de Uso</h2>`;
    const metrics = conversationState.metrics || { estimatesGiven: 0, leadsSaved: 0, errorsEncountered: 0 };
...
// Adiciona um botão para mostrar/esconder o dashboard de leads
function toggleLeadDashboard() {
    const dashboard = document.getElementById('lead-dashboard');
    if (dashboard) {
        dashboard.style.display = dashboard.style.display === 'none' ? 'block' : 'none';
    } else {
        displayLeadDashboard(); // Cria e exibe se não existir
    }
    // Pode-se adicionar lógica para mostrar/esconder métricas também
}

// Adiciona um botão para alternar a visibilidade do dashboard no rodapé ou cabeçalho
document.addEventListener('DOMContentLoaded', () => {
     const container = document.querySelector('.container');
     if (container) {
        const toggleButton = document.createElement('button');
        toggleButton.classList.add('btn', 'btn-outline-info', 'mt-3');
        toggleButton.textContent = 'Ver/Ocultar Leads e Métricas';
        toggleButton.onclick = toggleLeadDashboard;

        // Adiciona o botão no final do container principal, antes do footer
     ...
     // Inicializa a exibição dos dashboards se houver dados
     if(conversationState.leads && conversationState.leads.length > 0) {
         displayLeadDashboard();
     }
     updateMetricsDashboard(); // Garante que as métricas sejam exibidas se houver dados
});