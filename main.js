...
function handleUserResponse(response) {
    ...
    if (!validationError || questionData.defaultValue !== undefined || (finalResponse === '' && !validation.type === 'required')) {
...
// Adiciona listener para o botão de enviar quando a página carrega
document.addEventListener('DOMContentLoaded', () => {
    loadState(); // Carrega estado anterior se houver
    loadConfig(); // Carrega configurações e desenha botões de serviço

    // Adiciona um botão para mostrar/esconder o dashboard de leads e métricas
    const container = document.querySelector('.container');
    if (container) {
        const toggleButton = document.createElement('button');
        toggleButton.classList.add('btn', 'btn-outline-info', 'mt-3', 'mb-3'); // Estilo para o botão
     ...
     // Inicializa a exibição dos dashboards se houver dados salvos
...
// Permite enviar com Enter no campo de input
...
Bot: (Após cálculo e simulação de imagem)
     ...
     (Exibe a visualização simulada aqui)
